...
import static example.android.com.popularmovies.data.Constant.NETWORK_UPDATE_THRESHOLD_6_HOURS;
...
public class MovieSyncUtilities {
    ...
    public static void doNetworkUpdate(final Context context, @NonNull final String action) {
        AppExecutors.getInstance().diskIO().execute(new Runnable() {
            ...
            public void run() {
                ...
                boolean sixHoursPassedSinceLastNetworkUpdate = (timeSinceLastNetworkUpdate >=
                        TimeUnit.HOURS.toMillis(NETWORK_UPDATE_THRESHOLD_6_HOURS) );
    ...
    public static void startImmediateSync(@NonNull final Context context, @NonNull String action) {
        Intent fetchMoviesIntent = new Intent(context, MoviesIntentService.class);
        fetchMoviesIntent.setAction(action);
        context.startService(fetchMoviesIntent);
    }
